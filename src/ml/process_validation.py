...
class ProcessValidation():
    ...
    def draw_hists(self):
        cry_data, ce_mix_data = LoadML(run=self.run).get_full_results()
        draw = Draw()
        draw.plot_summary(cry_data["hists"], out_path = self.img_out_path / "h1o_3x3_cuts_CRY.png")
        draw.plot_summary(ce_mix_data["hists"], out_path = self.img_out_path / "h1o_3x3_cuts_CE_mix.png")