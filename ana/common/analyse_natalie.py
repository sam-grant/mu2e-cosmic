"""
This version follows the same cut flow as Natalie. 
"""
...
class Analyse:
    ...
    def define_cuts(self, data, cut_manager, on_spill=None):
        ...
        try:
            ...
            # 1. Reco track fit is electron 
            #
            is_reco_electron = selector.is_electron(data["trk"])
            data["is_reco_electron"] = is_reco_electron # & at_trk_front
            ...
            # 1.5 has_trk_front 
            # Natalie combines this with "is_reco_electron" 

            has_trk_front = ak.any(at_trk_front, axis=-1)
            data["has_trk_front"] = has_trk_front # & at_trk_front

            cut_manager.add_cut(
                name="has_trk_front", 
                description="Tracks pass through tracker entrance", 
                mask=has_trk_front 
            )
            ...
            # 11. CRV veto: |dt| < 150 ns (dt = coinc time - track t0) 
